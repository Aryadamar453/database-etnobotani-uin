(()=>{
  ...
  function renderLocations(){
    ...
    $('#location-grid').innerHTML=entries.map(([code,count],i)=>{
      const areaPlants=plants.filter(p=>p.locationCode===code);
      const pickIndexes=[0,Math.floor((areaPlants.length-1)/2),Math.max(0,areaPlants.length-1)];
      const picks=pickIndexes.map(n=>areaPlants[n]).filter(Boolean);
      const images=picks.map((p,j)=>{
        const src=photosFor(p)[j%3];
        return `<img src="${src}" alt="Dokumentasi ${escapeHtml(full[code]||code)}" loading="lazy">`;
      }).join('');
      return `<button class="location-card location-photo-card" data-location="${escapeHtml(code)}" aria-label="Buka katalog ${escapeHtml(full[code]||code)}">
        <span class="location-card-gallery" aria-hidden="true">${images}</span>
        <span class="location-card-shade" aria-hidden="true"></span>
        <span class="location-card-content">
          <small>KAWASAN ${String(i+1).padStart(2,'0')}</small>
          <strong>${escapeHtml(full[code]||code)}</strong>
          <span>${count} observasi · buka katalog →</span>
        </span>
      </button>`;
    }).join('');
